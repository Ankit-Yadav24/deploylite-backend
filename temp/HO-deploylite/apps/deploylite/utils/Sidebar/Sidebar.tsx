...
import Intregation from '../Header/SettingdComp/Intregation';
  ...
  const NavItems = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className="space-y-2">
      ...
      <Link href="/vultr" passHref>
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-r from-pink-600/0 to-purple-600/0 group-hover:from-pink-600/5 group-hover:to-purple-600/5 transition-all duration-300" />
          <SquareChevronDown className={cn("shrink-0 transition-all duration-300", 
            isMobile ? "h-6 w-6" : "h-5 w-5",
            isActiveSubRoute("/vultr") ? "text-pink-400" : "text-gray-400 group-hover:text-pink-300"
          )} />
          {(!isSidebarCollapsed || isMobile) && (
            <span className="ml-3 transition-all duration-300">Vultr Intregation</span>
          )}
          {isActiveSubRoute("/vultr") && (
            <div className="absolute right-2 w-1 h-6 bg-gradient-to-b from-pink-400 to-purple-400 rounded-full" />
          )}
